...
def main() -> int:
    ...
    parser.add_argument("--merge-overlaps", action="store_true", help="When using --all, merge overlapping/adjacent context windows")
        ...
        if args.lines:
            ...
            if args.all:
                if args.merge_overlaps:
                    ...
                    for match_line, start, end in matches:
                        if merged and start <= merged[-1][1] + 1:
                            prev_start, prev_end, prev_match_lines = merged[-1]
                            merged[-1] = (prev_start, max(prev_end, end), prev_match_lines + [match_line])
                        else:
                            merged.append((start, end, [match_line]))

                    for idx, (start, end, match_lines) in enumerate(merged, start=1):
                        joined = ", ".join(str(n) for n in match_lines)
                        print(f"=== MERGED MATCH WINDOW {idx}; match lines: {joined} ===")
                        selected = lines[start - 1:end]
                        print_block(path, selected, start, end, args.numbered)
                        print()
                else:
